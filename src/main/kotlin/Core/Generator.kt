...
class Generator(private val board: Board, private val maxBombs: Int)
{
    ...
    private fun searchAroundCell(x: Int, y: Int): Int
    {
        ...
        points.forEach {
            if(!board.insideBoard(it.x,it.y))
                return@forEach

            if(board.grid[it.x][it.y].value == -1)
                result++
        }
    ...
    fun generateAround(x: Int, y: Int) : ArrayList<Point>
    {
        ...
        for(xRel in -1..1) for(yRel in -1..1)
        {
    ...
    private fun generateUnusedCoords(x: Int, y: Int): Cell
    {
        ...
        do
        {
            ...
            if(board.grid[x2][y2].value == 0 && notAround(x,y,x2,y2))
            {
                unique = true
            }
    ...
    private fun notAround(x: Int,y: Int, x2:Int, y2:Int):Boolean
    {
        val xDelta = x-x2
        val yDelta = y-y2
